...
<template>
  <div
    v-if="isOpen"
    class="fixed z-50 inset-0 w-full h-full bg-black/50 flex items-center justify-center"
  >
    <div class="bg-gray-200 w-[50%] max-h-[500px] rounded-lg">
      <div class="flex justify-between border-b-2 border-gray-500">
        ...
        <button
          @click="close"
          class="text-white bg-gray-500 hover:bg-gray-700 rounded-full p-1 m-2"
        >
          <X />
        </button>
      </div>
      <div class="p-4 h-full">
		...
		:class="selectedImage ? 'min-h-12':'min-h-36 h-full'"
          ...
          autofocus
        ></textarea>
        <img class="w-full h-32 object-cover" v-if="selectedImage" :src="previewImage(selectedImage)" />
        <div>
          <div>
            <label
              for="image"
        ...
        <button @click="createPost" class="bg-blue-500 text-white p-2 rounded-lg w-full my-2">
          Post
        </button>
      </div>
    </div>
