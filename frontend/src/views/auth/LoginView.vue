...
<template>
    <div class="flex flex-col min-h-full justify-center px-6 py-12">
      ...
      <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        {{ errors }}
        <small class="text-red-500" v-if="Object.values(errors) != 0">{{errors}}</small>
        <form class="space-y-6" @submit.prevent>
          ...
          <div class="flex justify-center">
            <button type="button" @click="handleLogin" class="rounded-lg text-white bg-blue-700 px-4 py-2">
              Login
            </button>
          </div>
        </form>
